...
function polyline(pts, v, weight) {
  for (let i = 1; i < pts.length; i++) {
    ...
    if (weight >= 2)
      line(pts[i - 1].x, pts[i - 1].y + 1, pts[i].x, pts[i].y + 1, v);
  }
}
...
function drawEncoderKnob(ringIn, lockHit, turn) {
  ...
  line(
    CENTER_X + Math.cos(a) * 2,
    CENTER_Y + Math.sin(a) * 2,
    CENTER_X + Math.cos(a) * (outer - 3),
    CENTER_Y + Math.sin(a) * (outer - 3),
    1,
  );
}
  ...
  for (let i = 0; i < WORDMARK.length; i++) {
    ...
    for (let row = 0; row < glyph.length; row++) {
      for (let col = 0; col < glyph[row].length; col++) {
        if (glyph[row][col] === "1" && x + col <= revealX)
          plot(x + col, y + row, 1);
      }
    }
...
function frameParams(bootFrame) {
  ...
  const loopT =
    ((bootFrame - SPLASH_SETTLE_FRAME) % SPLASH_LOOP_FRAMES) /
    SPLASH_LOOP_FRAMES;
  return {
    signalIn: 1,
    ringIn: 1,
...
export function renderSplashScreen(state, deps) {
  ...
  const bootFrame = Math.floor(
    (state.splashFrameTick | 0) / SPLASH_TICKS_PER_FRAME,
  );
