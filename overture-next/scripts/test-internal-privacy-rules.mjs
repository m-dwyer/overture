...
} = require("./internal-privacy-rules.cjs");

const fixtureRoot = fs.mkdtempSync(
  path.join(os.tmpdir(), "overture-internal-rules-"),
);

fs.mkdirSync(path.join(fixtureRoot, "src/core/project/internal"), {
  recursive: true,
});
fs.mkdirSync(path.join(fixtureRoot, "src/view/internal"), { recursive: true });
fs.mkdirSync(path.join(fixtureRoot, "src/view/session/internal"), {
  recursive: true,
});

const rules = createInternalPrivacyRules({
  rootDir: fixtureRoot,
  sourceDir: "src",
});
const publicApiRules = createPublicApiRules({
  rootDir: fixtureRoot,
  sourceDir: "src",
});
const modules = discoverInternalModules({
  rootDir: fixtureRoot,
  sourceDir: "src",
});
...
assert.deepEqual(
  ...
  [
    "core-project-internals-stay-private",
    "view-internals-stay-private",
    "view-session-internals-stay-private",
  ],
);
...
assert.deepEqual(
  ...
  [
    ...
    { from: { pathNot: "^src/view/" }, to: { path: "^src/view/internal/" } },
    {
      ...
      to: { path: "^src/view/session/internal/" },
    },
  ],
);
...
assert.deepEqual(
  createPublicApiRules({ rootDir: fixtureRoot, sourceDir: "src" }),
  [
    {
      name: "view-public-api-only",
      severity: "error",
      comment:
        "Code outside src/view/ imports its public entry point, not implementation files.",
    ...
    },
    {
      ...
      severity: "error",
      comment:
        "Code outside src/view/session/ imports its public entry point, not implementation files.",
      from: { pathNot: "^src/view/session/" },
