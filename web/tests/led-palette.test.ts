...
import {
  OVERTURE_LED_COLOR,
  OVERTURE_LED_COLOR_VALUES,
} from "../../overture-next/src/ports/led-colors";
import {
  hasStaticLedPaletteEntry,
  ledColor,
  setLedPaletteEntryRGB,
} from "../src/led-palette.js";

describe("browser LED palette", () => {
  test("has intentional CSS mappings for every Overture-emitted LED color", () => {
    expect(
      OVERTURE_LED_COLOR_VALUES.filter(
        (color) => !hasStaticLedPaletteEntry(color),
      ),
    ).toEqual([]);
    expect(ledColor(OVERTURE_LED_COLOR.hint)).toBe("#ffd84d");
    expect(ledColor(OVERTURE_LED_COLOR.hint)).not.toBe(
      ledColor(OVERTURE_LED_COLOR.dim),
    );
    expect(ledColor(OVERTURE_LED_COLOR.hint)).not.toBe(
      ledColor(OVERTURE_LED_COLOR.active),
    );
    expect(ledColor(OVERTURE_LED_COLOR.hint)).not.toBe(
      ledColor(OVERTURE_LED_COLOR.selected),
    );
  });
