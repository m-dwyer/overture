...
export async function annotate(page: Page, action: string, showing: string, targets: Target[] = []): Promise<void> {
  const missing = await page.evaluate(({ gesture, show, controls }) => {
    globalThis.__OVT_MANUAL_GESTURE = gesture;
    // JSON legend [{ n, name }] — App renders numbered pills that match the badges.
    globalThis.__OVT_MANUAL_CONTROLS = JSON.stringify(controls.map((target, idx) => ({ n: idx + 1, name: target.name })));
    ...
    for (let i = 0; i < controls.length; i++) {
      ...
      const el = Array.from(document.querySelectorAll<HTMLElement>("[aria-label]")).find(
        (node) => node.getAttribute("aria-label") === target.aria
      );
      if (!el) {
...
export async function capture(page: Page, file: string, crop: CropMode = "panel"): Promise<void> {
