...
export const cheatSheet: CheatRow[] = [
  ...
  { control: "Side button", gesture: "Hold (Track View)", does: "Reveal that track's 16 clips on the step buttons" },
  { control: "Side buttons", gesture: "Tap (Session View)", does: "Launch the scene row (Shift = launch at next bar)" },
  { control: "Pads", gesture: "Tap", does: "Play notes; pick a drum lane on drum tracks; launch clips in Session View" },
  { control: "Pads", gesture: "Hold several + Step", does: "Enter a chord on that step" },
  { control: "Step 1-16", gesture: "Tap", does: "Place or clear a hit on the active lane/clip" },
  ...
  { control: "Delete + pad", gesture: "Hold + tap (Session)", does: "Clear a clip's notes (Shift = hard-reset)" },
  { control: "‹ ›", gesture: "Tap", does: "Page through the clip / loop view" },
  { control: "Octave +/-", gesture: "Tap", does: "Shift the pad octave / transpose" },
  { control: "Record", gesture: "Tap", does: "Arm a one-bar count-in, then records automatically — no Play press (tap again to stop)" },
  { control: "Play", gesture: "Tap", does: "Start / stop the transport" },
  { control: "Loop", gesture: "Tap / hold", does: "Performance-mode latch" },
  { control: "Shift + Note/Session", gesture: "Hold + tap", does: "Open the Global Menu (save, load, export, settings)" },
];
...
export const glossary: GlossaryRow[] = [
  ...
  { term: "Clip", def: "One pattern on a track. Each track holds 16 clips; the active clip is what Track View edits." },
  { term: "Parameter bank", def: "A page of eight encoder (K1-K8) parameters shown on the OLED (CLIP, NOTE FX, HARMONY, DELAY, SEQ ARP, ARP IN, AUTO)." },
  { term: "Drum lane", def: "One drum voice on a drum track; its 16 steps are edited on the step row. 32 lanes per drum track." },
  { term: "Step Edit", def: "The per-step editor opened by holding a step: length, velocity, micro-timing, and trig conditions." },
...
export const referenceGuide: GuideConfig = {
  ...
  editHint: "Edit web/tests/manual/reference.ts (walkthrough) and reference-content.ts (intro, cheat-sheet, glossary).",
