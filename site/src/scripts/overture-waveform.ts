...
const drawWaveform = (
  ...
  drawWaveLayer(group, startX, endX, centerX, ringIn, lock, drift, settledAmp + 2, 54, 3, 1.7, 3);
...
const drawEncoderKnob = (
  ...
  group.add(createEllipse(cx, cy, outer + 1, outer + 1, 9 + ringIn * 9, 1, 8));
  group.add(createEllipseStroke(cx, cy, outer, lerp(34, 218 + lockHit * 18, ringIn), 1, 9));
...
const animate = (state: WaveformState, time: number) => {
  ...
  renderFrame(state);
  window.requestAnimationFrame((nextTime) => animate(state, nextTime));
};

const initWaveform = (canvas: HTMLCanvasElement) => {
  ...
  window.requestAnimationFrame((time) => animate(state, time));
};

document
  .querySelectorAll<HTMLCanvasElement>("[data-overture-waveform]")
  .forEach((canvas) => initWaveform(canvas));
