...
lifecycle owners, Domain Intent router, and the command outbox. The Domain
Intent router routes by intent scope; Session, Track, Global, and Transport
handlers coordinate their workflow owners and emit commands such as
`track-note-on` and `track-note-off`. The runtime coordinates transport-driven
playback ticks and derives the Core Read Model; the host adapter converts Host
...
| Control input interpretation           | `src/application/controls/`                                                                                                    | `control-input-interpreter.ts`, `types.ts`                                                | Global and view control contexts                                                        | dependency-cruiser controls/intents rules     |
| Domain Intent handling                 | `src/application/intents/`                                                                                                     | Scoped intent types, `domain-intent-router.ts`, and scoped handlers                       | Handler-local workflow helpers                                                          | dependency-cruiser controls/intents rules     |
