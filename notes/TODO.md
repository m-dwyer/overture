...
- ~~**Move-native knob (CC 71-78) stutters sequencer in co-run.**~~ **RESOLVED.** Companion to the volume-knob fix (main `7398983`).

### Features

- ~~**Jog-click to toggle alt params on track banks** (instead of holding Shift)~~ **SHIPPED + DEVICE-VERIFIED 2026-05-26** (main `5567586`). Sticky `S.altMode` on a jog-click; down-arrow header icon (left of `Tr#`) steady when off, flashes ~2x/sec when on. Covers CLIP/DELAY/AUTOMATION (melodic) and DRUM LANE/REPEAT GROOVE/ALL LANES (drum); AUTOMATION's alt is **ASSIGN** mode (retarget CC/AT, all labels lit). Melodic SEQ ARP / ARP IN also get the arrow, reflecting the existing Arp Steps overlay. Perform-mode cycling (Velocity/Rpt1/Rpt2) is now solely on Shift+step-8. Holding Shift no longer affects bank alt-params. Also fixed a latent bug — Arp Steps overlay was firing on drum bank 5. Spec: `docs/superpowers/specs/2026-05-25-bank-altmode-jog-click-design.md`.
...
- ~~**Reclaim Back button** from Schwung suspend.~~ **NO-GO** — Back stays as Schwung's suspend gesture.
