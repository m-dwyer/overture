...
- **CC automation recording feels wrong** (noticed 2026-05-25) — while recording on the AUTO bank, knob values barely move and keep being reset. Confirmed **NOT** introduced by the aftertouch/AUTOMATION-bank work (that session's diff touches zero CC-recording lines); pre-dates it, in the CC automation redesign (`a8d9f36`). Likely suspects: run-length **knob acceleration** ("barely move"), and the **touch-suppression grace window** (`CC_TOUCH_GRACE_BLOCKS`) too short so playback re-asserts between detents, or the **resting-value loop reset** ("keeps resetting"). Needs on-device repro + investigation.
